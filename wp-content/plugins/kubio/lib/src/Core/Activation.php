...
class Activation {
	...
	public function activate() {
		...
		if ( ! $this->isCLI() ) {
			...
			$ai_stage2_with_free           = Flags::getSetting( 'aiStage2' ) && kubio_is_free();

			if ( kubio_is_fresh_site() || $is_unmodified_supported_theme ) {
				...
				$url = add_query_arg(
					array(
						'page' => 'kubio-get-started',
						'tab'  => $ai_stage2_with_free ? 'get-started' : 'website-starter',
					),
					admin_url( 'admin.php' )
				);
