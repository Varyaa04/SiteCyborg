...
class DemoSitesImporter {
	 ...
	 */
	private function setImporter( $config ) {
		...
		$this->importer = new WXRImporter(
			array(
				'fetch_attachments' => Arr::get( $config, 'fetch_attachments', true ),
				'is_stepped'        => Arr::get( $config, 'is_stepped', true ),
			),
			$this->logger
		);
